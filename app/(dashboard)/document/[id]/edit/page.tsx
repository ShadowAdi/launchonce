...
import { getDocumentById, updateDocument, updateFormMeta, updateFormFields } from "@/actions/document.action";
...
import { ArrowLeft, FileEdit, Plus, Edit2, Trash2, ChevronUp, ChevronDown, X, Save, GripVertical } from "lucide-react";
...
const steps = [
  ...
  {
    title: "Form (Optional)",
    description: "Update response form",
  },
];
...
export default function EditDocumentPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          form.reset({
            ...
            enableForm: doc.form ? true : false,
            formTitle: doc.form?.title || "",
            formDescription: doc.form?.description || "",
            listResponsesPublicly: doc.form?.listResponsesPublicly || false,
            formFields: [],
          });

          // Populate form fields if they exist
          if (doc.form) {
            setExistingFormId(doc.form.id);
            setFormFields(
              doc.form.fields.map((f) => ({
                id: f.id,
                label: f.label,
  ...
  const onSubmit = async (data: DocumentFormValues) => {
    ...
    try {
      ...
      // Handle form updates if enabled
      if (data.enableForm) {
        // Update form metadata
        const formMetaResult = await updateFormMeta(resolvedParams.id, user.id, {
        ...
        if (!formMetaResult.success) {
          toast.error("Failed to update form metadata");
          return;
        }

        // Compare existing fields with new fields to determine creates/updates/deletes
        ...
        // Find deleted field IDs (existed before but not in current formFields)
        ...
        if (creates.length > 0 || updates.length > 0 || deletes.length > 0) {
          const fieldsResult = await updateFormFields(resolvedParams.id, user.id, {
      ...
      } else if (existingFormId && !data.enableForm) {
        // If form was enabled but now disabled, just disable it
        await updateFormMeta(resolvedParams.id, user.id, {
          isEnabled: false,
        });
      }
